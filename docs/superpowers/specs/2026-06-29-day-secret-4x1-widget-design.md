...
| 10 | `ui/settings/...` | 新增"今日密码 桌面卡 已选地图"设置项（复用 Configure UI） |
...
`ui/settings/...` 增"今日密码 桌面卡" SectionHeader：
...
子屏复用 `DaySecretMapPickerContent` Composable，Configure Activity 与设置页共享。
...
@Test fun buildDaySecretLongMapName()  // map_name ellipsize=end (通过 RemoteViews 字段或 mock)
...
`WidgetContractSourceTest` 的 `nativeWidgetsStayCompactAndTextOnly` / `craftingLayoutsDeclareFourCountdownRowsInSeparateAndCombinedCards` 已检查 `<ImageButton>` 与 `<Chronometer>` 不出现。新卡片不使用这两个标签，**自动满足**。
...
- `ui/settings/...`（设置页子屏 + 共享 Composable）
